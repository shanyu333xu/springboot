...
public class BlogService {
    ...
    public List<Blog> selectTop() {
        ...
        blogList = blogList.stream().sorted((b1, b2) -> b2.getReadCount().compareTo(b1.getReadCount())) //按阅读量降序
                .limit(20)
    ...
    public Set<Blog> selectRecommend(Integer blogId) {
        Blog blog = this.selectById(blogId);//博客信息
        String tags = blog.getTags();
        //Set里面是唯一的 去重
        Set<Blog> blogSet = new HashSet<>();
        if (ObjectUtil.isNotEmpty(tags)) {
            List<Blog> blogList = this.selectAll(null);
            JSONArray tagsArr = JSONUtil.parseArray(tags);//转成json数组：包含多个tag
